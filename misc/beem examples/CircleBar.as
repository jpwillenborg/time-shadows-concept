package {
	...
	public class CircleBar extends Sprite {
		
		private var count:int = 0;
		
		
		public function CircleBar(color:String, idX:int) {
			
			if (Audio.hasStarted) {
				
				addChild(Atlas.generate("neon circle bar " + color));
		...
		protected function loop(evt:EnterFrameEvent):void {
			...
			if (y < Main.screenHeight * 0.5 && alpha < 100) {
				alpha += 0.20;
			}
			...
			if (alpha <= 0 || y > Main.screenHeight) { // /2
				removeSelf();
			}
			
			/*if (x < -width/2 - 50 || x > Main.screenWidth + width/2 + 50) {
				removeSelf();
			}*/
